...
pisos_punts <- read.csv("C:/30DayMapChallenge2025/Day1_Points/hut_comunicacio_opendata.csv", 
                        encoding="UTF-8",
                        dec=",")
...
sf_pisos <- st_as_sf(x = pisos_punts,
...
sf_pisos_25831 <- st_transform(x = sf_pisos, 
                               crs = 25831)

## Addició de les coordenades en columnes separades
sf_pisos_25831 <- cbind(sf_pisos_25831,
                        st_coordinates(sf_pisos_25831))

## Visualització de les dades
ggplot(data = sf_pisos_25831) +
geom_sf() +
coord_sf(datum = 25831,
         xlim = c(424239,434738), ylim = c(4578249,4589339))
...
ggplot() + 
  ...
  geom_sf(data = barris) + 
  geom_point(data = sf_pisos_25831, 
             mapping = aes(x = X, y = Y, colour = NOM_DISTRICTE), size = 0.1) + 
  coord_sf(datum = 25831)
