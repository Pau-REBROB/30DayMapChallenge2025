...
# Transformació en dataframe
df_sortides_bcn_2024 <- as.data.frame(sortides_bcn_2024)
...
df_passatgers_2024 <- subset(x = df_passatgers_2024, subset = grepl(pattern = 'BRD', x = tra_meas))
# Join dels dos df
df_vols_passatgers <- left_join(df_vols_2024, df_passatgers_2024, by = join_by(airp_pr))
...
# Extreure codi aeroport en nova columna
df_vols_passatgers_2024$codi_aeroport <- substr(x = df_vols_passatgers_2024$Sortida,
                                                start = 12,
                                                stop = 15)

# Unió amb geometries aeroports
df_geo <- left_join(df_vols_passatgers_2024, aeroports, by = join_by(codi_aeroport == AIRP_ICAO_))
...
sortides_sf <- cbind(sortides_sf, st_coordinates(sortides_sf))
